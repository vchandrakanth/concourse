...
describe('Surface Layer Data Creation Concourse ', async function () {
    let originalTimeout;
    ...
    it('Step 2: Create Azure Data For SurfaceLayer', async function (): Promise<any> {
        await surface.createDataForSurfaceLayer(surfaceName, azureAccount, 0, accountKey, keyValue);
    });

    it('Step 3: Create AWS Data For SurfaceLayer', async function (): Promise<any> {
        await surface.createDataForSurfaceLayer(surfaceName, awsAccount, 0, accountKey, keyValue);
    });

    it('Step 4: Update AWS Data For SurfaceLayer', async function (): Promise<any> {
        await surface.updateDataForSurfaceLayer(surfaceName, 2, 1, accountKey, keyValue);
    });

    it('Step 5: Delete AWS Account Data For SurfaceLayer', async function (): Promise<any> {
        await surface.deleteDataAccountForSurfaceLayer(surfaceName, awsAccount, 2);
    });

    it('Step 6: Delete AZURE Account Data For SurfaceLayer', async function (): Promise<any> {
        await surface.deleteDataAccountForSurfaceLayer(surfaceName, azureAccount, 1);
    });
